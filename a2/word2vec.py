...
def naiveSoftmaxLossAndGradient(
    ...
    # print(o_vector)
    predictions = softmax(outsideVectors @ centerWordVec)
    # print(predictions)
    loss = -np.log(predictions[outsideWordIdx])
    expected_o_vector = np.sum(predictions.reshape(-1, 1) * outsideVectors, axis=0)
    # print(expected_o_vector)
    gradCenterVec = -o_vector + expected_o_vector
    gradOutsideVecs = predictions.reshape(-1, 1) * centerWordVec.reshape(1, -1)  
    gradOutsideVecs[outsideWordIdx, :] = (predictions[outsideWordIdx] - 1) * centerWordVec
    # print((1-predictions[outsideWordIdx]) * centerWordVec)
